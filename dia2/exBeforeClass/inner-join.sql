...
SELECT 
    c.first_name, COUNT(a.address) AS endereços
FROM
    sakila.customer AS c
        INNER JOIN
...
-- 6. Monte uma query que exiba o nome , sobrenome e a média de valor ( amount ) paga aos funcionários no ano de 
-- 2006. Use as tabelas payment e staff . Os resultados devem estar agrupados pelo nome e sobrenome do funcionário.
SELECT
	s.first_name,
    s.last_name,
    AVG(p.amount) AS avarege_amount
FROM
	sakila.staff AS s
		INNER JOIN
